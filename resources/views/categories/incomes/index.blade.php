    ...
    <x-slot name="content">
        ...
        <div class="p-1 border rounded-md shadow m-1" x-data="{newCategory:false, text: 'New'}">
            <div class="flex flex-row justify-between text-sm">
                <h1 class="p-1 font-bold">Income Categories</h1>
                <div>
            ...
            <div class="pt-3">
                @error(session('income_category'))
                    <p class="text-sm text-red-500 text-white pl-2">{{ $message }}</p>
                @enderror
                <div style="display:none;" x-show="newCategory">
                    <form class="flex flex-row p-2" action="{{ route('income-categories.store') }}" method="POST">
                        @csrf
                        <div class="w-5/6">
                            <input class="w-full border rounded-md outline-none px-4" type="text" name="income_category" placeholder="New category...">
                        </div>
                        <div class="w-1/6 ">
